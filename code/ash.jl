...
# the fitted mixture weights (x) and the runtime of the likelihood
# computations (t_likelihood), model fitting (t_fit) and posterior
# computations (t_posterior).
function ash(x, s; method = "mixSQP", gridmult = 1.3, lowrank = "svd",
  ...
  out, t_likelihood, bytes, gctime, memallocs = @timed begin
  ...
  out, t_posterior, bytes, gctime, memallocs = @timed begin
    ind = findall(w .> 0);
    ps2 = sd[ind].^2;
    s2  = s.^2;
    t   = s2 .+ ps2';
    ...
    comp_post_prob  = comp_post_prob ./ sum(comp_post_prob,dims = 2);
    post_mean       = sum(comp_post_prob .* comp_post_mean,dims = 2);
    post_mean2      = sum(comp_post_prob .* comp_post_mean2,dims = 2);
  end
    
  return Dict([("x",x),
               ("timing-likelihood",t_likelihood),
               ("timing-fit",t_fit),
               ("timing-posterior",t_posterior)])
end
